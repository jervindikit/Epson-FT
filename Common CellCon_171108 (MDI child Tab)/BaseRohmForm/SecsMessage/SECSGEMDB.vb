...
Public Class SECSGEMDB
    ...
    Public Shared Sub LoadLinkReportTableToDictionary(ByVal sourceTable As DataTable, _
                                               desDictionary As Dictionary(Of String, SecsDataType))  '160722 \783  common secs data type

        desDictionary.Clear()

        Dim linkedReport As SecsDataType
        Dim ceid As UInt32
        Dim rptId As String
        ...
        For Each row As DataRow In sourceTable.Rows
            ceid = CUInt(row("CEID"))
            rptId = row("RPTID")
            If desDictionary.ContainsKey(ceid) Then
                linkedReport = m_LinkedReportDic(ceid)
            Else
                linkedReport = New SecsDataType
                desDictionary.Add(ceid, linkedReport)
            End If
            linkedReport.RPTID.Add(rptId)
            linkedReport.CEID = ceid
        Next
    ...
    Public Shared Sub LoadDefineReportTableToDictionary(ByVal sourceTable As DataTable, _
                                                 desDictionary As Dictionary(Of String, SecsDataType))  '160722 \783  common secs data type

        Dim rptId As String
        Dim definedReport As SecsDataType
        Dim vid As String
